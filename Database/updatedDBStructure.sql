...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 29, 2015 at 07:33 PM
...
--
-- Stand-in structure for view `mercreports`
--
CREATE TABLE IF NOT EXISTS `mercreports` (
`Voucher No` varchar(7)
,`Phone` varchar(11)
,`Customer` varchar(20)
,`Status` varchar(15)
...
--
DROP TABLE IF EXISTS `mercreports`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mercreports` AS select `b`.`rcptNo` AS `Voucher No`,`b`.`phone` AS `Phone`,`c`.`clientName` AS `Customer`,`b`.`status` AS `Status`,`b`.`issueDate` AS `IssueDate`,`b`.`deldate` AS `deldate`,sum((`info`.`qty` * (select `cl`.`countVal` from `clothes` `cl` where (`cl`.`cid` = `info`.`cid`)))) AS `qty`,(`b`.`netAmount` - `b`.`gst`) AS `grossAmount`,`b`.`gst` AS `gst`,`b`.`netAmount` AS `NetAmount` from ((`booking` `b` join `booking_clothes` `info`) join `client` `c`) where ((`info`.`rcptNo` = `b`.`rcptNo`) and (`b`.`phone` = `c`.`phone`)) group by `b`.`rcptNo`;
