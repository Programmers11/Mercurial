...
public class Printer implements Printable {
    ...
    //prints the string text
    void printfwithY(String text, String fontname, int fontsize, int type, int Xcord, int Ycord) {
        font = new Font(fontname, type, fontsize);
        Style = new AttributedString(text);
        Style.addAttribute(TextAttribute.FONT, font);
        graphics1.drawString(Style.getIterator(), Xcord, Ycord);
 ...
 //</editor-fold>
    ...
    private void headers(int pageIndex, boolean isCashMemo) {
        ...
        if (pageIndex == 0) {
            ...
            boolean ReadyLinePrinted = printReadyReceipts();
                        //printfwithY("ok", "Verdana", 8, Font.BOLD, xcord, ycord-10);

            if (ReadyLinePrinted) {
                printfimgHead(img, 8, ycord - 9, 88, 190);
            } else {
...
//               noPagesLeft=true;
    ...
    private boolean printReadyReceipts() {
        ...
        if (readyReceipt > 0) {
            try {
                ...
                in.nextLine();
                String customText = in.nextLine();
                System.out.println("custom text == " + customText);

                printfwithY(customText + " " + readyReceipt, "Verdana", 8, Font.PLAIN, xcord+4, ycord - 12);
